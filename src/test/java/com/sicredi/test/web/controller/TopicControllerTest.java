...
public class TopicControllerTest {
	...
	public void shouldCallCreatePollWhenDoNotExist() {
		//given
		long topicId = 1234L;
		PollDto pollDto = mock(PollDto.class);
		Topic topic = mock(Topic.class);
	...
	public void shouldThrowExceptionWhenPollIsExpired() {
		...
		long topicId = 1234L;
		VoteDto voteDto = mock(VoteDto.class);
		doThrow(ExpiredTopicException.class).when(topicValidator).
			validateForVote(anyLong(), anyString(), any());
		given(voteDto.getUsername()).willReturn("user");
		given(voteDto.getVoteOption()).willReturn(VoteOption.YES);
	...
	public void shouldThrowExceptionWhenTopicIsOpen() {
		//given
		long topicId = 1234L;
		doThrow(OpenTopicException.class).when(topicValidator).
			validatePollForGetResults(anyLong());
